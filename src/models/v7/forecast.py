...
class ForecastStatus(enum.Enum):
    """Status enumeration for forecast."""
    DRAFT = "draft"
    ACTIVE = "active"
    ARCHIVED = "archived"
    DELETED = "deleted"
...
@dataclass
class Forecast:
    """Data model for forecast result with confidence intervals."""
    ...
    def activate(self) -> None:
        ...
        if self.status == ForecastStatus.DELETED:
    ...
    def soft_delete(self) -> None:
        ...
        self.status = ForecastStatus.DELETED
        self.updated_at = datetime.utcnow()
    ...
    def from_dict(cls, data: dict[str, Any]) -> "Forecast":
        ...
        if data.get("status"):
            instance.status = ForecastStatus(data["status"])
        if data.get("metadata"):
            instance.metadata = data["metadata"]
        return instance

    def __repr__(self) -> str:
        return f"Forecast(id={self.id[:8]}, name={self.name!r}, status={self.status.value})"
