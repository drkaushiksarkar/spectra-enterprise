...
@dataclass
class User:
    """Data model for user and permission model."""
    ...
    def from_dict(cls, data: dict[str, Any]) -> "User":
        ...
        if data.get("status"):
            instance.status = UserStatus(data["status"])
        if data.get("metadata"):
            instance.metadata = data["metadata"]
        return instance

    def __repr__(self) -> str:
        return f"User(id={self.id[:8]}, name={self.name!r}, status={self.status.value})"
