...
class NotificationStatus(enum.Enum):
    """Status enumeration for notification."""
    DRAFT = "draft"
    ACTIVE = "active"
    ARCHIVED = "archived"
    DELETED = "deleted"
...
@dataclass
class Notification:
    """Data model for notification preference and delivery."""
    ...
    def from_dict(cls, data: dict[str, Any]) -> "Notification":
        ...
        if data.get("status"):
            instance.status = NotificationStatus(data["status"])
        if data.get("metadata"):
            instance.metadata = data["metadata"]
        return instance

    def __repr__(self) -> str:
        return f"Notification(id={self.id[:8]}, name={self.name!r}, status={self.status.value})"
