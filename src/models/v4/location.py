...
class LocationStatus(enum.Enum):
    """Status enumeration for location."""
    DRAFT = "draft"
    ACTIVE = "active"
    ARCHIVED = "archived"
    DELETED = "deleted"
...
@dataclass
class Location:
    """Data model for geographic location hierarchy."""
    ...
    def activate(self) -> None:
        ...
        if self.status == LocationStatus.DELETED:
    ...
    def soft_delete(self) -> None:
        ...
        self.status = LocationStatus.DELETED
        self.updated_at = datetime.utcnow()
    ...
    def from_dict(cls, data: dict[str, Any]) -> "Location":
        ...
        if data.get("status"):
            instance.status = LocationStatus(data["status"])
        if data.get("metadata"):
            instance.metadata = data["metadata"]
        return instance

    def __repr__(self) -> str:
        return f"Location(id={self.id[:8]}, name={self.name!r}, status={self.status.value})"
