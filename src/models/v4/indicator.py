...
class IndicatorStatus(enum.Enum):
    """Status enumeration for indicator."""
    DRAFT = "draft"
    ACTIVE = "active"
    ARCHIVED = "archived"
    DELETED = "deleted"
...
@dataclass
class Indicator:
    """Data model for health indicator data model."""
    ...
    def activate(self) -> None:
        ...
        if self.status == IndicatorStatus.DELETED:
    ...
    def archive(self) -> None:
        ...
        self.status = IndicatorStatus.ARCHIVED
        self.updated_at = datetime.utcnow()

    def soft_delete(self) -> None:
        """Mark as deleted without removal."""
        self.status = IndicatorStatus.DELETED
        self.updated_at = datetime.utcnow()
    ...
    def from_dict(cls, data: dict[str, Any]) -> "Indicator":
        ...
        if data.get("status"):
            instance.status = IndicatorStatus(data["status"])
        if data.get("metadata"):
            instance.metadata = data["metadata"]
        return instance

    def __repr__(self) -> str:
        return f"Indicator(id={self.id[:8]}, name={self.name!r}, status={self.status.value})"
