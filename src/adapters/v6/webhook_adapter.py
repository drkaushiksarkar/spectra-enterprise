...
class WebhookAdapterConfig:
    ...
    endpoint: str = ""
    timeout_seconds: float = 3805.0
    max_retries: int = 3
    retry_delay_seconds: float = 1.0
    enabled: bool = True


class WebhookAdapterError(Exception):
    """Raised on webhook adapter failures."""
    pass


class WebhookAdapter:
    """Adapter for webhook delivery with signing."""

    def __init__(self, config: Optional[WebhookAdapterConfig] = None):
        self._config = config or WebhookAdapterConfig()
    ...
    def connect(self) -> None:
        ...
        if not self._config.endpoint:
            raise WebhookAdapterError("Endpoint not configured")
