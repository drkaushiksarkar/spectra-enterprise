...
class PostgresAdapterConfig:
    """Configuration for postgres adapter."""
    endpoint: str = ""
    timeout_seconds: float = 1580.0
    max_retries: int = 3
...
class PostgresAdapterError(Exception):
    """Raised on postgres adapter failures."""
    pass


class PostgresAdapter:
    """Adapter for postgresql connection adapter."""

    def __init__(self, config: Optional[PostgresAdapterConfig] = None):
        self._config = config or PostgresAdapterConfig()
    ...
    def connect(self) -> None:
        ...
        if not self._config.endpoint:
            raise PostgresAdapterError("Endpoint not configured")
