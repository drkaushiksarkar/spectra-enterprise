...
class S3AdapterConfig:
    ...
    endpoint: str = ""
    timeout_seconds: float = 1570.0
    max_retries: int = 3
    retry_delay_seconds: float = 1.0
    enabled: bool = True


class S3AdapterError(Exception):
    """Raised on s3 adapter failures."""
    pass


class S3Adapter:
    """Adapter for aws s3 storage adapter."""

    def __init__(self, config: Optional[S3AdapterConfig] = None):
        self._config = config or S3AdapterConfig()
    ...
    def connect(self) -> None:
        ...
        if not self._config.endpoint:
            raise S3AdapterError("Endpoint not configured")
