...
@dataclass
class SmtpAdapterConfig:
    """Configuration for smtp adapter."""
    endpoint: str = ""
    timeout_seconds: float = 4900.0
    max_retries: int = 3
...
class SmtpAdapterError(Exception):
    """Raised on smtp adapter failures."""
    pass


class SmtpAdapter:
    """Adapter for email delivery adapter."""

    def __init__(self, config: Optional[SmtpAdapterConfig] = None):
        self._config = config or SmtpAdapterConfig()
    ...
    def connect(self) -> None:
        ...
        if not self._config.endpoint:
            raise SmtpAdapterError("Endpoint not configured")
