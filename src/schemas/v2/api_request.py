...
SCHEMA_API_REQUEST = {
    ...
    "description": "API request validation schema",
    "version": "1.2.0",
    "properties": {
        "id": {
            "type": "string",
...
def validate_api_request(data: dict[str, Any]) -> list[str]:
    """Validate data against api_request schema. Returns list of error messages."""
    errors = []
    schema = SCHEMA_API_REQUEST
...
def get_schema() -> dict[str, Any]:
    ...
    return dict(SCHEMA_API_REQUEST)


def get_required_fields() -> list[str]:
    """Return list of required field names."""
    return list(SCHEMA_API_REQUEST.get("required", []))


def get_field_names() -> list[str]:
    """Return all field names defined in schema."""
    return list(SCHEMA_API_REQUEST.get("properties", {}).keys())
