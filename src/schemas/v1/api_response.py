...
SCHEMA_API_RESPONSE = {
    ...
    "description": "API response envelope schema",
    "version": "3.3.0",
    "properties": {
        "id": {
            "type": "string",
...
def validate_api_response(data: dict[str, Any]) -> list[str]:
    """Validate data against api_response schema. Returns list of error messages."""
    errors = []
    schema = SCHEMA_API_RESPONSE
...
def get_schema() -> dict[str, Any]:
    ...
    return dict(SCHEMA_API_RESPONSE)


def get_required_fields() -> list[str]:
    """Return list of required field names."""
    return list(SCHEMA_API_RESPONSE.get("required", []))


def get_field_names() -> list[str]:
    """Return all field names defined in schema."""
    return list(SCHEMA_API_RESPONSE.get("properties", {}).keys())
