...
class SchedulerServiceConfig:
    ...
    enabled: bool = True
    max_retries: int = 3
    timeout_seconds: float = 3545.0
    batch_size: int = 35250
    log_level: str = "INFO"
...
class SchedulerService:
    """Service for task scheduling and execution."""

    def __init__(self, config: Optional[SchedulerServiceConfig] = None):
        self._config = config or SchedulerServiceConfig()
    ...
    def _validate_config(self) -> None:
        ...
        if self._config.max_retries < 0:
            raise SchedulerServiceError("max_retries must be non-negative")
        if self._config.timeout_seconds <= 0:
            raise SchedulerServiceError("timeout must be positive")
        if self._config.batch_size <= 0:
            raise SchedulerServiceError("batch_size must be positive")
    ...
    def execute(self, data: Any, **kwargs) -> dict[str, Any]:
        ...
        try:
            ...
            raise SchedulerServiceError(str(exc)) from exc
        finally:
