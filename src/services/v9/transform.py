...
class TransformConfig:
    ...
    enabled: bool = True
    max_retries: int = 3
    timeout_seconds: float = 5145.0
    batch_size: int = 51250
    log_level: str = "INFO"


class TransformError(Exception):
    """Raised when transform encounters an error."""
    pass


class Transform:
    """Service for etl transformation pipeline."""
    ...
    def _validate_config(self) -> None:
        ...
        if self._config.max_retries < 0:
            raise TransformError("max_retries must be non-negative")
        if self._config.timeout_seconds <= 0:
            raise TransformError("timeout must be positive")
        if self._config.batch_size <= 0:
            raise TransformError("batch_size must be positive")
