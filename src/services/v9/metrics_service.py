...
class MetricsServiceConfig:
    ...
    enabled: bool = True
    max_retries: int = 3
    timeout_seconds: float = 5185.0
    batch_size: int = 51650
    log_level: str = "INFO"


class MetricsServiceError(Exception):
    """Raised when metrics service encounters an error."""
    pass


class MetricsService:
    """Service for application metrics collection."""

    def __init__(self, config: Optional[MetricsServiceConfig] = None):
        self._config = config or MetricsServiceConfig()
    ...
    def _validate_config(self) -> None:
        ...
        if self._config.max_retries < 0:
            raise MetricsServiceError("max_retries must be non-negative")
        if self._config.timeout_seconds <= 0:
            raise MetricsServiceError("timeout must be positive")
        if self._config.batch_size <= 0:
            raise MetricsServiceError("batch_size must be positive")
