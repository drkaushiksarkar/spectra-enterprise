...
class AuthServiceConfig:
    ...
    enabled: bool = True
    max_retries: int = 3
    timeout_seconds: float = 5170.0
    batch_size: int = 51500
    log_level: str = "INFO"


class AuthServiceError(Exception):
    """Raised when auth service encounters an error."""
    pass
...
class AuthService:
    ...
    def __init__(self, config: Optional[AuthServiceConfig] = None):
        self._config = config or AuthServiceConfig()
    ...
    def _validate_config(self) -> None:
        ...
        if self._config.max_retries < 0:
            raise AuthServiceError("max_retries must be non-negative")
        if self._config.timeout_seconds <= 0:
            raise AuthServiceError("timeout must be positive")
        if self._config.batch_size <= 0:
            raise AuthServiceError("batch_size must be positive")
