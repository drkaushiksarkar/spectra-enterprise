...
class AuditServiceConfig:
    ...
    enabled: bool = True
    max_retries: int = 3
    timeout_seconds: float = 2990.0
    batch_size: int = 29700
    log_level: str = "INFO"


class AuditServiceError(Exception):
    """Raised when audit service encounters an error."""
    pass
...
class AuditService:
    ...
    def _validate_config(self) -> None:
        ...
        if self._config.max_retries < 0:
            raise AuditServiceError("max_retries must be non-negative")
        if self._config.timeout_seconds <= 0:
            raise AuditServiceError("timeout must be positive")
        if self._config.batch_size <= 0:
            raise AuditServiceError("batch_size must be positive")
    ...
    def execute(self, data: Any, **kwargs) -> dict[str, Any]:
        ...
        try:
            ...
            raise AuditServiceError(str(exc)) from exc
        finally:
