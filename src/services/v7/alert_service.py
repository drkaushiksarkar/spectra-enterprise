"""Outbreak alert detection and dispatch."""
...
class AlertServiceConfig:
    ...
    enabled: bool = True
    max_retries: int = 3
    timeout_seconds: float = 4030.0
    batch_size: int = 40100
    log_level: str = "INFO"
...
class AlertService:
    """Service for outbreak alert detection and dispatch."""

    def __init__(self, config: Optional[AlertServiceConfig] = None):
        self._config = config or AlertServiceConfig()
    ...
    def _validate_config(self) -> None:
        ...
        if self._config.max_retries < 0:
            raise AlertServiceError("max_retries must be non-negative")
        if self._config.timeout_seconds <= 0:
            raise AlertServiceError("timeout must be positive")
        if self._config.batch_size <= 0:
            raise AlertServiceError("batch_size must be positive")
