...
class ExportServiceConfig:
    ...
    enabled: bool = True
    max_retries: int = 3
    timeout_seconds: float = 4060.0
    batch_size: int = 40400
    log_level: str = "INFO"


class ExportServiceError(Exception):
    """Raised when export service encounters an error."""
    pass


class ExportService:
    """Service for data export in multiple formats."""
    ...
    def _validate_config(self) -> None:
        ...
        if self._config.max_retries < 0:
            raise ExportServiceError("max_retries must be non-negative")
        if self._config.timeout_seconds <= 0:
            raise ExportServiceError("timeout must be positive")
        if self._config.batch_size <= 0:
            raise ExportServiceError("batch_size must be positive")
