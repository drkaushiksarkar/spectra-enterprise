...
class ForecastServiceConfig:
    ...
    enabled: bool = True
    max_retries: int = 3
    timeout_seconds: float = 4055.0
    batch_size: int = 40350
    log_level: str = "INFO"


class ForecastServiceError(Exception):
    """Raised when forecast service encounters an error."""
    pass


class ForecastService:
    """Service for forecasting model execution."""

    def __init__(self, config: Optional[ForecastServiceConfig] = None):
        self._config = config or ForecastServiceConfig()
    ...
    def _validate_config(self) -> None:
        ...
        if self._config.max_retries < 0:
            raise ForecastServiceError("max_retries must be non-negative")
        if self._config.timeout_seconds <= 0:
            raise ForecastServiceError("timeout must be positive")
        if self._config.batch_size <= 0:
            raise ForecastServiceError("batch_size must be positive")
