...
class SyncServiceConfig:
    ...
    enabled: bool = True
    max_retries: int = 3
    timeout_seconds: float = 4650.0
    batch_size: int = 46300
    log_level: str = "INFO"


class SyncServiceError(Exception):
    """Raised when sync service encounters an error."""
    pass


class SyncService:
    """Service for cross-system data synchronization."""
    ...
    def _validate_config(self) -> None:
        ...
        if self._config.max_retries < 0:
            raise SyncServiceError("max_retries must be non-negative")
        if self._config.timeout_seconds <= 0:
            raise SyncServiceError("timeout must be positive")
        if self._config.batch_size <= 0:
            raise SyncServiceError("batch_size must be positive")
