...
class NotificationServiceConfig:
    ...
    enabled: bool = True
    max_retries: int = 3
    timeout_seconds: float = 4615.0
    batch_size: int = 45950
    log_level: str = "INFO"
...
class NotificationService:
    ...
    def _validate_config(self) -> None:
        ...
        if self._config.max_retries < 0:
            raise NotificationServiceError("max_retries must be non-negative")
        if self._config.timeout_seconds <= 0:
            raise NotificationServiceError("timeout must be positive")
        if self._config.batch_size <= 0:
            raise NotificationServiceError("batch_size must be positive")
