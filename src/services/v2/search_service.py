...
class SearchServiceConfig:
    ...
    enabled: bool = True
    max_retries: int = 3
    timeout_seconds: float = 1330.0
    batch_size: int = 13100
    log_level: str = "INFO"
...
class SearchService:
    """Service for full-text and vector search."""
    ...
    def _validate_config(self) -> None:
        ...
        if self._config.max_retries < 0:
            raise SearchServiceError("max_retries must be non-negative")
        if self._config.timeout_seconds <= 0:
            raise SearchServiceError("timeout must be positive")
        if self._config.batch_size <= 0:
            raise SearchServiceError("batch_size must be positive")
